...
client.on('messageDelete', message =>{ // ивент, когда удаляется любое сообщение с сервера https://discord.js.org/#/docs/main/stable/class/Client?scrollTo=e-guildMemberAdd
  ...
  client.channels.cache.get("1155224088088944840").send(embed); // айди вашего канала с логами
})
...
client.on('guildMemberAdd', member =>{ // ивент, когда пользователь присоединяется к серверу https://discord.js.org/#/docs/main/stable/class/Client?scrollTo=e-guildMemberAdd
    ...
    Ты наш \`${client.guilds.get("1040257933646250104").memberCount}\` участник! **`) // айди вашего сервера               !!!!!!!!!!
    ...
    .setColor('RANDOM')
    member.send(embed);
    client.channels.cache.get('1155224088088944840').send(embed2) // айди вашего канала с логами
})
...
async function change() {
    let members = client.guilds.cache.get("1155223639411667026").memberCount // сколько людей на сервере + указать айди своего сервера
    client.channels.cache.get("1040257933646250104").setName(`На сервере: ${members}`); // свой айди войса
}

var interval = setInterval(function () { change(); }, 20000  ); // время обновления в миллисекундах

client.login('MTE1NTIxNjMwNjc0MDY2NjQ4MA.GLPUvq.8_lTHcx4SAxm3U2wlD9ydYAqvXuhU6mhUIOusM') // токен вашего бота
